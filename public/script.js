document.addEventListener('DOMContentLoaded', async () => {
    ...
    const init = async () => {
        ...
        try {
            const response = await fetch('/src/routers.json');
    ...
    const handleSubmitQuery = async () => {
        ...
        await handleApiRequest(apiUrlWithParams,
            currentApiData.name);
    };
    ...
    const handleApiRequest = async (apiUrl,
        ...
        try {
            ...
            if (contentType && contentType.includes('image/')) {
                ...
                video.src = videoUrl;
                video.controls = true;
                video.className = 'response-video video-fluid rounded shadow-sm fade-in';
                ...
                audio.controls = true;
                audio.paused = true;
                audio.className = 'response-audio audio-fluid rounded shadow-sm fade-in';
                ...
                downloadBtn.download = `${apiName.toLowerCase().replace(/\s+/g, '-')}.${blob.type.split('/')[1] || 'mp3'}`;
                downloadBtn.className = 'btn btn-primary mt-3 w-100';
                downloadBtn.innerHTML = '<i class="fas fa-download me-2"></i> Download Audio';

                DOM.modal.content.appendChild(audio);
                DOM.modal.content.appendChild(downloadBtn);
